  ...
  public $ADDRESS;
	...
	public function setProducts($products) {
		...
		foreach($products as $product) {
			$item_amount = $product['AMOUNT'] * $product['QTY'];
			$total += $item_amount;
			$item = array(
				"L_NAME".$i => $product['NAME'],
				"L_AMT".$i  => $product['AMOUNT'],
				"L_QTY".$i  => $product['QTY'],
				"L_DESC".$i => $product['DESCRIPTION']
			);
			$product_string = http_build_query($item);
			$i++;
		}
	...
	public function doPayment($paymentDetails) {
		$nvpArray = array(
			"TOKEN" => urlencode($paymentDetails['TOKEN']),
			"PAYERID" => urlencode($paymentDetails['PAYERID']),
			"PAYMENTACTION" => "Sale",
			"AMT" => urlencode($paymentDetails['AMT']),
			"CURRENCYCODE" => urlencode($paymentDetails['CURRENCYCODE']),
			"IPADDRESS" => urlencode($_SERVER['SERVER_NAME'])
		);
	  $nvpstr = "&".http_build_query($nvpArray);
	  $resultArray = $this->hash_call("DoExpressCheckoutPayment", $nvpstr);
	  return $resultArray;
	}
